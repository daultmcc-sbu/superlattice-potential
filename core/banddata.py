...
class BandData:
    def __init__(self, model, xv, yv, band, in_bz=None):
        Hs = make_hamiltonians_single(model, xv, yv, DELTAK)
        self.model = model
        self.band = band
        self.xv = xv
        self.yv = yv
        ...
        if model.trivial_basis:
            self.below, self.at, self.above, self.berry, self.g11, self.g22, self.g12 = fast_energies_and_qgt_raw(Hs, band, DELTAK)
        else:
            ffs = make_form_factors_single(model, xv, yv, DELTAK)
            self.below, self.at, self.above, self.berry, self.g11, self.g22, self.g12 = fast_energies_and_qgt_raw_ff(Hs, ffs, band, DELTAK)
...
@nb.guvectorize(
    ['(complex64[:,:,:], int32, float32, float32[:], float32[:], float32[:], float32[:], float32[:], float32[:], float32[:])',
     '(complex128[:,:,:], int64, float64, float64[:], float64[:], float64[:], float64[:], float64[:], float64[:], float64[:])'],
    '(m,n,n),(),()->(),(),(),(),(),(),()',
    target='parallel', cache=True
)
def fast_energies_and_qgt_raw(Hs, band, deltak, below, at, above, berry, g11, g22, g12):
    ...
    berry[0] = deltak2 * np.angle(link0010 * link1011 * link1101 * link0100)

    g11_ = 2 * deltak2 * (1 - np.abs(link0010))
    g22_ = 2 * deltak2 * (1 - np.abs(link0100))
    g11[0] = g11_
    g22[0] = g22_
    g12[0] = deltak2 * (1 - np.abs(link0011)) - 0.5 * (g11_ + g22_)

@nb.guvectorize(
    ['(complex64[:,:,:], complex64[:,:,:], int32, float32, float32[:], float32[:], float32[:], float32[:], float32[:], float32[:], float32[:])',
     '(complex128[:,:,:], complex128[:,:,:], int64, float64, float64[:], float64[:], float64[:], float64[:], float64[:], float64[:], float64[:])'],
    '(m,n,n),(l,n,n),(),()->(),(),(),(),(),(),()',
    target='parallel', cache=True
)
def fast_energies_and_qgt_raw_ff(Hs, ffs, band, deltak, below, at, above, berry, g11, g22, g12):
    ceigvals, ceigvecs = np.linalg.eigh(Hs[0,:,:])
    below[0] = ceigvals[band-1]
    at[0] = ceigvals[band]
    above[0] = ceigvals[band+1]
    ...
    link0010 = np.vdot(vec00, ffs[0] @ vec10)
    link1011 = np.vdot(vec10, ffs[1] @ vec11)
    link1101 = np.vdot(vec11, ffs[2] @ vec01)
    link0100 = np.vdot(vec01, ffs[3] @ vec00)
    link0011 = np.vdot(vec00, ffs[4] @ vec11)

    deltak2 = 1 / deltak / deltak
    berry[0] = deltak2 * np.angle(link0010 * link1011 * link1101 * link0100)
...
def make_hamiltonians_single(model, xv, yv, deltak):
    xo = np.array([0, deltak, deltak, 0])
    yo = np.array([0, 0, deltak, deltak])
    return model.hamiltonian(xv[...,np.newaxis] + xo, yv[...,np.newaxis] + yo)

def make_form_factors_single(model, xv, yv, deltak):
    xo = deltak * np.array([0, 1, 1, 0, 0])
    yo = deltak * np.array([0, 0, 1, 1, 0])
    qx = deltak * np.array([1, 0, -1, 0, 1])
    qy = deltak * np.array([0, 1, 0, -1, 1])
    return model.form_factor(xv[...,np.newaxis] + xo, yv[...,np.newaxis] + yo, qx, qy)

def make_hamiltonians_sweep(modelf, paramvs, xv, yv, deltak):
    ...
    for params in it:
        model = modelf(*params)
        Hs[it.multi_index] = make_hamiltonians_single(model, xv, yv, deltak)
    return Hs
